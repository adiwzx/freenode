...
let userID = '22288ebf-deb0-46c9-9524-f1634e14f87d';

const proxyIPs = ['artuger.com', 'time.is', 'ip.sb', 'cdn.anycast.eu.org'];
...
function createVLESSSub(userID_Path, hostName) {
	let portArray_http = [80, 8080, 8880, 2052];
	let portArray_https = [443, 8443, 2053, 2096];
		...
		if (!hostName.includes('pages.dev')) {
			...
			portArray_http.forEach((port) => {
				...
				proxyIPs.forEach((proxyIP) => {
					const vlessSecHttp = `vless://${userID}@${proxyIP}${commonUrlPart_http}-${proxyIP}`;
